      ...
      <div className="mx-auto w-full max-w-7xl">
        ...
        <div className={`mt-12 grid grid-cols-1 gap-x-12 gap-y-14 lg:mt-14 lg:gap-x-14 lg:gap-y-16 ${galleryGrid}`}>
          {items.map((screenshot, index) => (
            <motion.article key={index} data-screenshot-index={index} className="flex min-w-0 flex-col items-center text-center" initial={reducedMotion ? false : { opacity: 0, y: 28 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.12 }} transition={{ duration: reducedMotion ? 0 : 0.62, delay: reducedMotion ? 0 : (index % 3) * 0.09, ease: [0.22, 1, 0.36, 1] }}>
              <motion.div className="flex w-full justify-center" whileHover={reducedMotion ? undefined : { y: -7, scale: 1.016, transition: { duration: 0.3, ease: [0.22, 1, 0.36, 1] } }}>
                <DeviceMockup screenSrc={screenshot.src} screenAlt={`${screenshot.title} ${dictionary.gallery.screenAltSuffix}`} deviceLabel={dictionary.accessibility.deviceMockup} screenPosition={screenshot.screenPosition} className="w-full max-w-[17.25rem] sm:max-w-[18.25rem] xl:max-w-[19.5rem]" />
              </motion.div>
